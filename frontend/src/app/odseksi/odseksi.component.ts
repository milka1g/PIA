...
@Component({
  selector: 'app-odseksi',
  templateUrl: './odseksi.component.html',
  styleUrls: ['./odseksi.component.css']
})
export class OdseksiComponent implements OnInit {
  ...
  ngOnInit(): void {
    //alert("pravim");
    this.predmetiService.dohvatiSvePredmeteSmera("si").subscribe((preds:Predmet[])=>{
      this.predmeti = preds;
    ...
    this.planAngServis.dohvatiSveZaNastavnika(this.user.username).subscribe((sve:PlanAng[])=>{
      ...
      //ocisti ove sto su rti 
      // for(let i = 0; i<this.sviPredmetiZap.length;i++){
      //   if(this.sviPredmetiZap[i].odsek!="si")
      //   this.sviPredmetiZap.splice(i,1);
      // }
    });
  }
